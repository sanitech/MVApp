...
const RoomCreate = () => {
  ...
  const handleRoomCapacityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const roomCapacity = parseFloat(e.target.value);
    setFormData({
      ...formData,
      roomCapacity: isNaN(roomCapacity) ? 0 : roomCapacity,
    });
  };
  ...
  const handleRoomCreate = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    try {
      const response = await axios.post("/v1/room", formDataToSend, {
        headers: {
      ...
      <div className="max-w-6xl mx-auto">
        <form onSubmit={handleRoomCreate}>
          <div className="bg-white rounded-xl shadow ">
            <h1 className="font-medium">Room create</h1>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {roomCategory.map((room) => (
                      <option key={room.category_id} value={room.category_id}>
                        {room.category_name}
                      </option>
                    ))}
                  </select>
